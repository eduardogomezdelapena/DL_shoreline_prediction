...
def index_mielke(s, o):
    """
	index of agreement
	Modified Mielke (circa 2015? Duveiller) 
	input:
        s: simulated
        o: observed
    output:
...
def cnn_custom(train_x, train_y, dev_x, dev_y, cfg):
    ...
    # fit model
    es = EarlyStopping(patience=10, verbose=2, min_delta=min_delta, monitor='val_loss', mode='auto', restore_best_weights=True)
...
cfg_list=pd.read_csv('./Data/10best_hyp_Wavesonly_Mielke_Hybrid.csv')
...
#Uncomment to save 
# yresults.to_csv('./Data/Hybrid_ensemble2.csv')
